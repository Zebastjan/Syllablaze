...
class AudioRecorder(QObject):
    ...
    def __init__(self):
        ...
        try:
            ...
            if "jack server is not running" in jack_errors:
                logger.info(
                    "Jack server not available - using alternative audio backend"
                )
    ...
    def start_recording(self):
        ...
        try:
            ...
            if sample_rate_mode == SAMPLE_RATE_MODE_WHISPER:
                ...
                logger.info(
                    f"Using Whisper-optimized sample rate: {target_sample_rate}Hz"
                )
                ...
                default_sample_rate = int(device_info["defaultSampleRate"])
                logger.info(
                    f"Using device's default sample rate: {default_sample_rate}Hz"
                )
    ...
    def _process_recorded_audio(self):
        ...
        try:
            ...
            logger.info(
                f"Recording processed in memory (length: {len(audio_data)} samples)"
            )
    ...
    def _get_original_sample_rate(self):
        ...
        if (
            hasattr(self, "current_sample_rate")
            and self.current_sample_rate is not None
        ):
