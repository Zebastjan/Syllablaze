...
class WhisperModelManager:
    ...
    def get_model_path(self, model_name):
        ...
        # Format: models--Systran--faster-distil-whisper-{name}
        ...
        faster_distil_model_dir = os.path.join(
            self.models_dir, f"models--Systran--faster-distil-whisper-{suffix}"
        )
        if os.path.exists(faster_distil_model_dir):
            return faster_distil_model_dir
    ...
    def query_huggingface_models(self):
        ...
        try:
            ...
            try:
                    ...
                    for model in data:
                        ...
                        if model_id.startswith("distil-whisper/"):
                            ...
                            if not any(
                                ...
                                for suffix in ["-ct2", "-ggml", "-openai", "-ONNX"]
                            ):
                                distil_models.append(model_name)

                    logger.info(
                        f"Found {len(distil_models)} distil-whisper models from Hugging Face API"
                    )

            except Exception as e:
                logger.warning(f"Failed to query Hugging Face API: {e}")
                # Fall back to hardcoded list of supported distil models (only those with Systran CTranslate2 versions)
                distil_models = [
                    "distil-medium.en",
                    "distil-large-v2",
                    "distil-small.en",
                ...
                logger.info(
                    f"Using fallback list of {len(distil_models)} supported distil-whisper models"
                )
    ...
    def get_model_info(self):
        ...
        for model_name in available_models:
            model_info[model_name] = self.get_single_model_info(
    ...
    def is_model_downloaded(self, model_name):
        ...
        # Check for Systran's CTranslate2-converted distil-whisper models
        # Format: models--Systran--faster-distil-whisper-{name}
        # Note: Strip 'distil-' prefix from model name (distil-medium.en -> medium.en)
        suffix = (
            model_name.replace("distil-", "", 1)
            if model_name.startswith("distil-")
            else model_name
        ...
        logger.info(f"  - Faster Distil-Whisper directory: {faster_distil_model_dir}")
        ...
        faster_distil_exists = os.path.exists(faster_distil_model_dir)
    ...
    def load_model(self, model_name):
        ...
        try:
            ...
            os.environ["HF_HUB_ENABLE_HF_TRANSFER"] = "0"
            logger.info(
                "Could not check for hf_transfer, using standard download method"
            )
        ...
        try:
            ...
            logger.error(
                "Failed to import faster_whisper. Please install it with: pip install faster-whisper>=1.1.0"
            )
            raise ImportError(
                "faster_whisper is not installed. Please install it with: pip install faster-whisper>=1.1.0"
            )
        ...
        compute_type_map = {"float32": "float32", "float16": "float16", "int8": "int8"}
        ...
        try:
            ...
            try:
                ...
                if hasattr(ctranslate2.models, "Whisper") and hasattr(
                    ctranslate2.models.Whisper, "get_model_path"
                ):
                    # Try to get the model path from the catalog
                    model_path = ctranslate2.models.Whisper.get_model_path(
                ...
                logger.warning(
                    f"CTranslate2 model catalog not available or doesn't support this model: {e}"
                )
            ...
            if not is_downloaded:
                logger.info(
                    f"Model {model_name} not found locally, attempting to download..."
                )
                try:
                    model = WhisperModel(
                        model_name,
            ...
            try:
                ...
                try:
                    ...
                    if importlib.util.find_spec("hf_transfer") is not None:
                        ...
                        os.environ["HF_HUB_ENABLE_HF_TRANSFER"] = "0"
                        logger.info(
                            "hf_transfer not available, using standard download method"
                        )
                except ImportError:
                    # If importlib.util is not available, disable hf_transfer
                    os.environ["HF_HUB_ENABLE_HF_TRANSFER"] = "0"
                ...
                try:
                    ...
                    logger.info(
                        f"Using CTranslate2 model catalog to download {model_name}"
                    )

                    if progress_callback:
                        progress_callback(
                            20,
                            f"Downloading {model_name} using CTranslate2 model catalog...",
                        )

                    # Download the model using CTranslate2 model catalog
                    model_path = ctranslate2.models.Whisper.download(
                        model_name=model_name, saving_directory=models_dir
                    )
                    ...
                    if progress_callback:
                        progress_callback(
                            90, "Model downloaded successfully, finalizing..."
                        )
                    ...
                    logger.info(
                        "Falling back to Faster Whisper's built-in download mechanism"
                    )
                    ...
                    if model_type == "distil":
                        ...
                        # WhisperModel() tries to both download AND load with CTranslate2,
                        # which fails for repos using safetensors format (no model.bin).
                        repo_id = model_info.get("repo_id")
                        if not repo_id:
                            error_msg = f"Repository ID not found for Distil-Whisper model '{model_name}'"
                            logger.error(error_msg)
                        ...
                        logger.info(
                            f"Downloading Distil-Whisper model: {model_name} (repo_id: {repo_id})"
                        )
                        ...
                        try:
                            ...
                            if progress_callback:
                                progress_callback(
                                    40,
                                    f"Downloading {model_name} using huggingface_hub...",
                                )
                            ...
                            logger.info(
                                f"Successfully downloaded {model_name} using huggingface_hub"
                            )
                        except Exception as hub_error:
                            logger.error(
                                f"Error downloading with huggingface_hub: {hub_error}"
                            )
                            if progress_callback:
                        ...
                        logger.info(
                            f"Downloading model: {model_name} from Hugging Face Hub"
                        )
                        WhisperModel(
                            model_name,
                            device="cpu",
