...
class FasterWhisperTranscriptionWorker(QThread):
    ...
    def _fallback_to_cpu(self, beam_size, vad_filter, word_timestamps):
        """Fallback to CPU when GPU/CUDA fails"""
        ...
        try:
            ...
            self.progress.emit("Retrying transcription on CPU...")

            self._segments_list, info = self.model.transcribe(
                self.audio_data,
                beam_size=beam_size,
            ...
            raise RuntimeError(f"Both GPU and CPU transcription failed: {e}")
    ...
    def run(self):
        try:
            ...
            word_timestamps = self.settings.get(
            ...
            try:
                ...
                self._segments_list = list(segments)
                ...
                if is_cuda_oom:
                    logger.error(f"CUDA OOM during transcription: {error_msg}")
                    logger.info("Attempting CPU fallback...")
                    self._fallback_to_cpu(beam_size, vad_filter, word_timestamps)
                else:
                    raise

            if self._cpu_fallback_attempted:
                segments_list = self._segments_list
            else:
                segments_list = self._segments_list
...
class WhisperTranscriber(QObject):
        ...
        if load_model:
            try:
                ...
                if self.model_manager.is_model_downloaded(self.current_model_name):
                    logger.info(
                        f"Loading model {self.current_model_name} which is already downloaded"
                    )
                    self.load_model()
                else:
                    logger.info(
                        f"Model {self.current_model_name} is not downloaded, deferring loading"
                    )
            except Exception as e:
    ...
    def reload_model_if_needed(self):
        ...
        if (
            not hasattr(self, "current_model_name")
            or model_name != self.current_model_name
        ):
            logger.info(
                f"Model changed from {getattr(self, 'current_model_name', 'None')} to {model_name}, reloading..."
            )
            self.load_model()
            return True
    ...
    def update_language(self, language=None):
        ...
        if language != self.current_language:
            ...
            for widget in app.topLevelWidgets():
                ...
                if isinstance(widget, QSystemTrayIcon) and hasattr(
                    widget, "update_tooltip"
                ):
                    widget.update_tooltip()
    ...
    def _prepare_for_transcription(self):
        ...
        if current_language != self.current_language:
            logger.info(
                f"Language changed from {self.current_language} to {current_language}, updating..."
            )
        ...
        lang_str = (
            "auto-detect" if self.current_language == "auto" else self.current_language
        )
    ...
    def transcribe(self, audio_data):
        ...
        try:
            ...
            print(
                f"Transcribing with model: {self.current_model_name}, language: {lang_str}"
            )
    ...
    def transcribe_audio(self, normalized_audio_data):
        ...
        if model_reloaded:
            logger.info(
                "Model was reloaded due to settings change before transcription"
            )
        ...
        print(
            f"Transcribing audio with model: {self.current_model_name}, language: {lang_str}"
        )

        self.worker = FasterWhisperTranscriptionWorker(
            self.model, normalized_audio_data
        )
