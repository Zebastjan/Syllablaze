...
from blaze.constants import (
    ...
    APPLET_MODE_OFF,
    APPLET_MODE_PERSISTENT,
    APPLET_MODE_POPUP,
    DEFAULT_APPLET_MODE,
    POPUP_STYLE_NONE,
    POPUP_STYLE_TRADITIONAL,
    POPUP_STYLE_APPLET,
    DEFAULT_POPUP_STYLE,
    ...
    def init_default_settings(self):
        ...
        if self.settings.value("clipboard_diagnostics") is None:
            self.settings.setValue(
                "clipboard_diagnostics", DEFAULT_CLIPBOARD_DIAGNOSTICS
            )
        ...
        if self.settings.value("recording_dialog_x") is None:
            self.settings.setValue(
                "recording_dialog_x", None
            )  # None = let window manager decide
        ...
        # Language mode settings for multi-backend STT
        # Default to system language (not multilingual)
        if self.settings.value("language_multilingual") is None:
            self.settings.setValue("language_multilingual", False)
        if self.settings.value("language_specific") is None:
            ...
            if lang_code not in self.VALID_LANGUAGES or lang_code == "auto":
                lang_code = "en"

            self.settings.setValue("language_specific", lang_code)
    ...
    def get(self, key, default=None):
        ...
        if key in integer_settings:
            ...
            if isinstance(value, str) and (value == "" or "@Invalid" in value):
                logger.debug(
                    f"Setting {key} has invalid/empty value: {value!r}, returning default: {default}"
                )
                return default
            try:
                return int(value)
                ...
                if key == "beam_size":
                    logger.warning(
                        f"Invalid beam_size in settings: {value!r}, using default: {DEFAULT_BEAM_SIZE}"
                    )
                    return DEFAULT_BEAM_SIZE
                elif key == "mic_index":
                    logger.warning(
                        f"Invalid mic_index in settings: {value!r}, using default: {default}"
                    )
                    return default
                logger.debug(
                    f"Cannot convert {key}={value!r} to int, returning default: {default}"
                )
                return default
        ...
        if key == "model":
            ...
            try:
                ...
                if beam_size < 1 or beam_size > 10:
                    logger.warning(
                        f"Invalid beam_size in settings: {value}, using default: {DEFAULT_BEAM_SIZE}"
                    )
                    return DEFAULT_BEAM_SIZE
                return beam_size
            except (ValueError, TypeError):
    ...
    def set(self, key, value):
        ...
        if key == "model":
            ...
            try:
                ...
                if beam_size < 1 or beam_size > 10:
                    raise ValueError(
                        f"Invalid beam_size: {value}. Must be between 1 and 10."
                    )
                value = beam_size
            except (ValueError, TypeError):
            ...
            if "+" not in value and len(value) > 1:
                raise ValueError(
                    f"Invalid shortcut format: {value}. Use format like 'Alt+Space'"
                )
